...
class Attribute {
  ...
  public:
    ...
    Attribute(const Constant *p_constant) : m_type(Tag::kConstantValue), m_constant_value_ptr(p_constant) {}
...
class SymbolEntry {
  ...
  public:
    ...
    SymbolEntry(const std::string &p_name, const KindEnum kind, const size_t level, const PType *const p_type,
                const Constant *const p_constant)
        : m_name(p_name), m_kind(kind), m_level(level), m_p_type(p_type), m_attribute(p_constant)
    {
    }

    SymbolEntry(const std::string &p_name, const KindEnum kind, const size_t level, const PType *const p_type,
                const FunctionNode::DeclNodes *const p_parameters)
        : m_name(p_name), m_kind(kind), m_level(level), m_p_type(p_type), m_attribute(p_parameters)
    {
    }
...
class SymbolTable {
  ...
  public:
    ...
    SymbolEntry *addSymbol(const std::string &p_name, const SymbolEntry::KindEnum kind, const size_t level,
...
class SymbolManager {
  ...
  public:
    ...
    friend SymbolEntry *genericAddSymbol(SymbolManager &p_manager, const std::string &p_name,
                                         const SymbolEntry::KindEnum kind, const PType *const p_type,
                                         const AttributeType *const p_attribute);

    SymbolEntry *addSymbol(const std::string &p_name, const SymbolEntry::KindEnum kind, const PType *const p_type,
                           const Constant *const p_constant);
    SymbolEntry *addSymbol(const std::string &p_name, const SymbolEntry::KindEnum kind, const PType *const p_type,
                           const FunctionNode::DeclNodes *const p_parameters);
    ...
    void reconstructHashTableFromSymbolTable(const SymbolTable *const p_table) const;
    void removeSymbolsFromHashTable(const SymbolTable *const p_table) const;

  private:
    std::pair<bool, SymbolEntry *> checkExistence(const std::string &p_name, const size_t current_level) const;
};
