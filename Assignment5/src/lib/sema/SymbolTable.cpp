...
// > SymbolTable
// ===========================================
SymbolEntry *SymbolTable::addSymbol(const std::string &p_name, const SymbolEntry::KindEnum kind, const size_t level,
                                    const PType *const p_type, const Constant *const p_constant)
{
    m_entries.emplace_back(new SymbolEntry(p_name, kind, level, p_type, p_constant));
    return m_entries.back().get();
}

SymbolEntry *SymbolTable::addSymbol(const std::string &p_name, const SymbolEntry::KindEnum kind, const size_t level,
                                    const PType *const p_type, const FunctionNode::DeclNodes *const p_parameters)
{
    m_entries.emplace_back(new SymbolEntry(p_name, kind, level, p_type, p_parameters));
    return m_entries.back().get();
}
...
static void dumpSymbolTable(const SymbolTable *const table)
{
    ...
    auto construct_attr_string = [&type_string](const auto &p_entry_ptr) {
        if (p_entry_ptr->getKind() == SymbolEntry::KindEnum::kFunctionKind) {
            const FunctionNode::DeclNodes *const parameters_ptr = p_entry_ptr->getAttribute().parameters();
    ...
    auto dump_entry = [&construct_attr_string](const auto &p_entry_ptr) {
        ...
        std::printf("%-11s", kKindStrings[static_cast<size_t>(p_entry_ptr->getKind())]);
        std::printf("%lu%-10s", p_entry_ptr->getLevel(), (p_entry_ptr->getLevel() != 0) ? "(local)" : "(global)");
...
void SymbolManager::reconstructHashTableFromSymbolTable(const SymbolTable *const p_table) const
{
    if (!p_table) {
        return;
    }
    ...
    auto construct_entry_on_hash_map = [&](const auto &p_entry_ptr) {
        auto existence_pair = checkExistence(p_entry_ptr->getName(), p_entry_ptr->getLevel());
        ...
        if (existence_pair.second) {
            ...
            m_hash_entries.emplace(std::piecewise_construct, std::forward_as_tuple(p_entry_ptr->getName()),
                                   std::forward_as_tuple(p_entry_ptr.get()));
        }
    };

    for_each(p_table->getEntries().begin(), p_table->getEntries().end(), construct_entry_on_hash_map);
}

void SymbolManager::removeSymbolsFromHashTable(const SymbolTable *const p_table) const
{
    if (!p_table) {
        return;
    }
    ...
    auto remove_entry_from_hash_map = [&](const auto &p_entry_ptr) {
        ...
        assert(hash_search_result != m_hash_entries.end() && "CANNOT remove the symbol that doesn't exist");
    ...
    for_each(p_table->getEntries().begin(), p_table->getEntries().end(), remove_entry_from_hash_map);
}

void SymbolManager::prevScope()
{
    assert(m_current_table && "If happens, it means that the uses of popScope() are more than the"
                              "ones of pushScope()");
...
std::pair<bool, SymbolEntry *> SymbolManager::checkExistence(const std::string &p_name,
                                                             const size_t current_level) const
{
    ...
    if (search_result != m_hash_entries.end()) {
        ...
        if (old_entry->getLevel() == current_level || old_entry->getKind() == SymbolEntry::KindEnum::kLoopVarKind) {
...
SymbolEntry *genericAddSymbol(SymbolManager &p_manager, const std::string &p_name, const SymbolEntry::KindEnum kind,
                              const PType *const p_type, const AttributeType *const p_attribute)
{
    auto existence_pair = p_manager.checkExistence(p_name, p_manager.getCurrentLevel());
    ...
    auto *new_entry =
        p_manager.m_current_table->addSymbol(p_name, kind, p_manager.m_current_level, p_type, p_attribute);
    ...
    if (existence_pair.second) {
        ...
        p_manager.m_hash_entries.emplace(std::piecewise_construct, std::forward_as_tuple(p_name),
...
SymbolEntry *SymbolManager::addSymbol(const std::string &p_name, const SymbolEntry::KindEnum kind,
                                      const PType *const p_type, const Constant *const p_constant)
{
...
SymbolEntry *SymbolManager::addSymbol(const std::string &p_name, const SymbolEntry::KindEnum kind,
                                      const PType *const p_type, const FunctionNode::DeclNodes *const p_parameters)
{
    return genericAddSymbol<FunctionNode::DeclNodes>(*this, p_name, kind, p_type, p_parameters);
}
