...
class FunctionInvocationNode final : public ExpressionNode {
  ...
  public:
    ...
    FunctionInvocationNode(const uint32_t line, const uint32_t col, const char *const p_name, ExprNodes &p_args)
        : ExpressionNode{line, col}, m_name(p_name), m_args(std::move(p_args))
    {
    }

    const char *getNameCString() const { return m_name.c_str(); }
    int getCurrentDimension() { return m_args.size(); }

    ExprNodes &getArgs() { return m_args; }
