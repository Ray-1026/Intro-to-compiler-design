...
class VariableNode final : public AstNode {
  ...
  public:
    ...
    VariableNode(const uint32_t line, const uint32_t col, const std::string &p_name, const PTypeSharedPtr &p_type,
                 const std::shared_ptr<ConstantValueNode> &p_constant_value_node)
        : AstNode{line, col}, m_name(p_name), m_type(p_type), m_constant_value_node_ptr(p_constant_value_node)
    {
    }
    ...
    bool checkDimension() { return m_type->checkDimension(); }
    void setType(std::string type) { m_constant_value_node_ptr->setType(type); }
};
