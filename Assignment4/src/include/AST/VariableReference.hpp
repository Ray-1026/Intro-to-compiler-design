...
class VariableReferenceNode final : public ExpressionNode {
  ...
  public:
    ...
    // array reference
    VariableReferenceNode(const uint32_t line, const uint32_t col, const char *const p_name, ExprNodes &p_indices)
        : ExpressionNode{line, col}, m_name(p_name), m_indices(std::move(p_indices))
    {
    }
    ...
    ExpressionNode *errorIndex();

    int getCurrentDimension() { return m_indices.size(); }
    void updateDimension(std::string type);
};
