...
void AstDumper::incrementIndentation() { m_indentation += m_indentation_stride; }

void AstDumper::decrementIndentation() { m_indentation -= m_indentation_stride; }

static void outputIndentationSpace(const uint32_t indentation) { std::printf("%*s", indentation, ""); }
...
void AstDumper::visit(ConstantValueNode &p_constant_value)
{
    ...
    std::printf("constant <line: %u, col: %u> %s\n", p_constant_value.getLocation().line,
                p_constant_value.getLocation().col, p_constant_value.getVal());
}
...
void AstDumper::visit(FunctionNode &p_function)
{
    ...
    std::printf("function declaration <line: %u, col: %u> %s %s\n", p_function.getLocation().line,
                p_function.getLocation().col, p_function.funcName(), p_function.funcProto());
...
void AstDumper::visit(FunctionInvocationNode &p_func_invocation)
{
    ...
    std::printf("function invocation <line: %u, col: %u> %s\n", p_func_invocation.getLocation().line,
                p_func_invocation.getLocation().col, p_func_invocation.getFuncName());
...
void AstDumper::visit(VariableReferenceNode &p_variable_ref)
{
    ...
    std::printf("variable reference <line: %u, col: %u> %s\n", p_variable_ref.getLocation().line,
                p_variable_ref.getLocation().col, p_variable_ref.getName());
